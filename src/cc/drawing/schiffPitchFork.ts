...
export class SchiffPitchFork extends ThreePointDrawing {
	static type = 'SchiffPitchFork'
	...
	_recalculateLines() {
		...
		let origin = {x: _x1, y: (_y1 + _y2) / 2}

		let midPt = Vector.midPoint({x: _x2, y: _y2}, {x: _x3, y: _y3})
		let midRemotepoint = BaseLine.remotePointOfRay(origin.x, origin.y, midPt.x, midPt.y, width, height)
		let midLine = new Line({x1: origin.x, y1: origin.y, x2: midRemotepoint.x, y2: midRemotepoint.y})
		...
		for (let i = 0; i < this.levels.length; i++) {
			...
			let leftOffset = BaseLine.verticalOffsetOfParallelLine(origin.x, origin.y, midPt.x, midPt.y, leftX, leftY)
			let rightOffset = BaseLine.verticalOffsetOfParallelLine(origin.x, origin.y, midPt.x, midPt.y, rightX, rightY)

			let remotePointLeft = BaseLine.remotePointOfRay(origin.x, origin.y + leftOffset, midPt.x, midPt.y + leftOffset, width, height)
			let remotePointRight = BaseLine.remotePointOfRay(origin.x, origin.y + rightOffset, midPt.x, midPt.y + rightOffset, width, height)
